...
public class OTTunnel {
    ...
    private String bestEffortEnable = "";
        ...
        try {
            ...
            if("tunnels".equals(el.getName())){
                for(Iterator<Element> i=el.elementIterator();i.hasNext();){
                    ...
                    if(null != domain.elementText("bestEffortEnable") && !"".equals(domain.elementText("bestEffortEnable"))){
                        tunnel.setBestEffortEnable(domain.elementText("bestEffortEnable"));
                    }
                    ...
                    if(null != ElTunnelPaths){
                        ...
                        for(Iterator<Element> j=ElTunnelPaths.elementIterator();j.hasNext();){
                            ...
                            if(null != tunnelPath.elementText("lspId") && !"".equals(tunnelPath.elementText("lspId"))){
                                oTTunnelPath.setId(tunnelPath.elementText("lspId"));
                            }
    ...
    public List<OTTunnel> parseOpsToUi(String content,OpsRestCaller client,Boolean allNot){
        ...
        try {
            ...
            for(Iterator<Element> i=ele.elementIterator();i.hasNext();){
                ...
                if(null != domain.elementText("bestEffortEnable") && !"".equals(domain.elementText("bestEffortEnable"))){
                    oTTunnel.setBestEffortEnable(domain.elementText("bestEffortEnable"));
                }
                //state
                String tunnelState = "";
                ...
                if(allNot){
                    ...
                    for(int k=0;k<TunnelPathsList.size();k++){
                        ...
                        if(null == oTTunnelPath.getPathName() || "".equals(oTTunnelPath.getPathName())){
                            continue;
                        }
                        OTPath oTPath = new OTPath();
    ...
    public List<OTPath> getLifePath(String content,String id){
        List<OTPath> list = new ArrayList<OTPath>();
        OTTunnel oTTunnel = this;
        try {
            Document doc = DocumentHelper.parseText(content);
            Element el = doc.getRootElement();
            Element ele = el.element("mplsTe").element("rsvpTeLsps");

            for(Iterator<Element> i=ele.elementIterator();i.hasNext();){
                Element domain = i.next();
                if(null != domain.elementText("tunnelName") && oTTunnel.getName().equals(domain.elementText("tunnelName"))){
                    if( (null != domain.elementText("mplsTunnelIngressLSRId") && oTTunnel.getIngressIp().equals(domain.elementText("mplsTunnelIngressLSRId")))
                            &&
                            (null != domain.elementText("mplsTunnelEgressLSRId") && oTTunnel.getEgressIp().equals(domain.elementText("mplsTunnelEgressLSRId")))
                            ){

                        Element mplsTunnelARHops = domain.element("mplsTunnelARHops");
                        if(null != mplsTunnelARHops){
                            OTPath oTPath = new OTPath();
                            oTPath.setName("best_effort");
                            oTPath.setType("best_effort");
                            List<OTPathHop> pathHopsList = new ArrayList<OTPathHop>();
                            for(Iterator<Element> j=mplsTunnelARHops.elementIterator();j.hasNext();){
                                Element mplsTunnelARHop = j.next();
                                if(null != mplsTunnelARHop.elementText("mplsTunnelInstance") && id.equals(mplsTunnelARHop.elementText("mplsTunnelInstance"))){
                                    OTPathHop oTPathHop = new OTPathHop();
                                    if(null != mplsTunnelARHop.elementText("mplsTunnelARHopIndex") && !"".equals(mplsTunnelARHop.elementText("mplsTunnelARHopIndex"))){
                                        oTPathHop.setId(mplsTunnelARHop.elementText("mplsTunnelARHopIndex"));
                                    }
                                    if(null != mplsTunnelARHop.elementText("mplsTunnelARHopIpAddr") && !"".equals(mplsTunnelARHop.elementText("mplsTunnelARHopIpAddr"))){
                                        oTPathHop.setNextIp(mplsTunnelARHop.elementText("mplsTunnelARHopIpAddr"));
                                    }
                                    pathHopsList.add(oTPathHop);
                                }
                            }
                            oTPath.setPathHops(pathHopsList);
                            list.add(oTPath);
                        }
                    }
                }
    ...
    public String getBestEffortEnable() {
        return bestEffortEnable;
    }
    public void setBestEffortEnable(String bestEffortEnable) {
        this.bestEffortEnable = bestEffortEnable;
    }
