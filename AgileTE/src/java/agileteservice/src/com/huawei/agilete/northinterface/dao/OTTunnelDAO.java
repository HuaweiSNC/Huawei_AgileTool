...
import com.huawei.agilete.base.servlet.MyMplsRsvpTeLsps;
...
public class OTTunnelDAO   implements IOTDao{
    ...
    public RetRpc edit(String content){
        ...
        for(OTTunnel oTTunnel:oTTunnel_.getTunnels()){
            ...
            body.append("<bestEffortEnable>").append(oTTunnel.getBestEffortEnable()).append("</bestEffortEnable>");
    ...
    public RetRpc getPaths(String apiPath){
        ...
        if(null != list && list.size() == 1){
            OTTunnel oTTunnel = list.get(0);
            Boolean lifePath = false;
            if("best_effort".equalsIgnoreCase(oTTunnel.getWhereFlow())){
                lifePath = true;
            }
            ...
            if(!lifePath){
                ...
                //逃生路径
                String id = "";
                for(int i =0;i<oTTunnel.getTunnelPaths().size();i++){
                    OTTunnelPath oTTunnelPath =oTTunnel.getTunnelPaths().get(i);
                    if("best_effort".equals(oTTunnelPath.getPathType())){
                        id = oTTunnelPath.getId();
                        break;
                    }
                }
                if(null != id && !"".equals(id)){
                    MyMplsRsvpTeLsps myMplsRsvpTeLsps = new MyMplsRsvpTeLsps(client);
                    String[] name = null;
                    if(!"".equals(apiPath)){
                        name = new String[]{apiPath};
                    }
                    RetRpc retRpc = myMplsRsvpTeLsps.get(name);
                    if(200 == retRpc.getStatusCode()){
                        pathsList = oTTunnel.getLifePath(retRpc.getContent(), id);
                    }
                }
            }
