...
//			open(keySpace);
	...
	public List<String[]> getByColumnNameRange(String columnFamily,String key,String stratColumnName,String endColumnName,boolean isReversed){
		...
		try{
		    ...
		    if(isReversed==false){
		    	sliceRange.start = StringtoByteBuffer(stratColumnName);
				sliceRange.finish = StringtoByteBuffer(endColumnName);
		    }else if(isReversed==true){
		    	sliceRange.finish = StringtoByteBuffer(stratColumnName);
				sliceRange.start = StringtoByteBuffer(endColumnName);
		    }
			sliceRange.reversed = isReversed;
		    predicate.setSlice_range(sliceRange);
	        List<ColumnOrSuperColumn> results = cassandraClient.get_slice(
	        		StringtoByteBuffer(key), parent, predicate, ConsistencyLevel.ONE);
	        for (ColumnOrSuperColumn column : results)
	        {
	            Column col = column.column;
	            result.add(new String[]{ByteBuffertoString(col.name),ByteBuffertoString(col.value)});
	        }
		}catch(Exception e){
			result = null;
			e.printStackTrace();
	...
	public List<String[]> getAllColumnByKey(String columnFamily,String key,boolean isReversed) throws Exception {
	List<String[]>result = null;
		try{
			result = new ArrayList<String[]>();
			ColumnParent parent = new ColumnParent(columnFamily);
...
//		System.out.println();
	...
	//	CassandraOperate operate = new CassandraOperate();
	}
}
