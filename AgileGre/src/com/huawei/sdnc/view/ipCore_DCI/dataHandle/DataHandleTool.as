package com.huawei.sdnc.view.ipCore_DCI.dataHandle
{
	import com.huawei.sdnc.controller.ipCoreController.GetNqaData;
...
//				device.flowStatices(classifiername,link);
				device.setFlowStaticsData(classifiername,link);
			...
			SdncUtil.flowStaticsTimer.deviceList=deviceListForFlowStatics;
			networkX.innerColorFunction=null;
			beginNode.alarmState.setNewAlarmCount(SdncUtil.FLOW_BEGIN,1);
			endNode.alarmState.setNewAlarmCount(SdncUtil.FLOW_END,1);
			//			var alarm:Alarm = new Alarm("0",beginNode.id,MOVING);
			//			var alarm1:Alarm = new Alarm("1",endNode.id,MOVING);
			//			networkX.elementBox.alarmBox.add(alarm);
			//			networkX.elementBox.alarmBox.add(alarm1);
		}
			...
			ebox.forEachByBreadthFirst(function(item:IData):void{
				if(item is StateNode){
					...
					for each(var tnl:Object in tnls){
						...
						myLink.setStyle(Styles.LINK_BUNDLE_GAP,1);
						ebox.add(myLink);
					}
				}
			});
...
//						device.addQosList(qosCalculate);
						...
						for each(var greTunnel:GreTunnel in gre.greTunnels){
							...
							//全局调用NQA的方法
							var getNqaData:GetNqaData=new GetNqaData;
							/*getNqaData.getNqaIfmResult(dstNode.getClient("ip"),myLink,stateNode);*/
							getNqaData.setNqaData(dstNode.getClient("ip"),myLink,stateNode);
							getNqaDataList.push(getNqaData);
							myLink.setClient("nqa",getNqaData);
//							device.getNqaIfmResult(dstNode.getClient("ip"),myLink);
			...
			SdncUtil.nqaTimer.nqaDataList=getNqaDataList;
		}
		/**
		 * 
		 * @param innerIp
...
//						}
		 ...
		 * 
		 */		
		public static function addOrDelLinkByTnl(deviceIp:String,dstIpAddr:String,method:String,networkX:NetworkX):void
		{
