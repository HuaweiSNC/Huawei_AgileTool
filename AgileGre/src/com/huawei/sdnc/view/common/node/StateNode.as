...
//			dispatchPropertyChangeEvent("cpuRate",__cpuRate,value);
		...
		public function updata(url:String):void
		{
			...
			else
			{
				...
				var url:String = "http://"+opsIp+"/devices/"+id1+"/devm/memoryInfos/memoryInfo";
				ConnUtil.getInstence().clientQuery(url,ConnUtil.METHOD_GET,getRamResult,onfaultRam);
				var uri:String = "http://"+opsIp+"/devices/"+id1+"/devm/cpuInfos";
...
//				ConnUtil.getInstence().clientQuery(url,ConnUtil.METHOD_GET,onDevmRestResult,onClientFault);
		...
		private function getCpuResult(e:HttpResponseEvent,data:ByteArray):void
		{
		...
		private function getRamResult(e:HttpResponseEvent,data:ByteArray):void
		{
		...
		private function onDevmRestResult(e:HttpResponseEvent,data:ByteArray):void
		{
...
//			ConnUtil.getInstence().clientQuery(url,ConnUtil.METHOD_GET,onMemInfoRestResult,onClientFault);
		...
		private function onCpuInfoRestResult(e:HttpResponseEvent,data:ByteArray):void
		{
			if(data)
			{
				var str:String = data.toString();
		...
		private function onMemInfoRestResult(e:HttpResponseEvent,data:ByteArray):void
		{
			if(data)
			{
				var str:String = data.toString();
