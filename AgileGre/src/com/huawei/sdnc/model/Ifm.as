...
{
	import com.huawei.sdnc.model.ifm.IfDynamicInfo;
	import com.huawei.sdnc.model.ifm.IfStatistics;
...
//		public static var timer:Timer = new Timer(3000);
		...
		public function getIfm(ifmIsReady:Function):void
		{
			...
			var aclUrl:String="http://"+opsIp+"/devices/"+id+"/ifm/interfaces/interface/ifmAm4/json";
...
//			});
		...
		private function onGetResult(e:HttpResponseEvent,data:ByteArray):void
		{
			...
			if(!errorcode.parse(e,data)){
				...
				DataHandleTool.showOnConsole(errorcode.printErrorMessage(stateNode, "   getIfm失败\n"));
				reFun();
				return;
			}
			try{
				  if(data.toString() == null||data.toString() == ""){
					  ...
					  DataHandleTool.showOnConsole("设备"+devicename+"   Ifm没有数据\n");
					  reFun();
					  return;
				  }
				   interfaces=[];
