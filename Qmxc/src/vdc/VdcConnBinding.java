...
public class VdcConnBinding {

    public final static String urlBody = "/vdc/tnlEndPoints/tnlEndPoint/tnlConnections/tnlConnection/connBindings/connBinding";
    public final static String fullUrlbody = "/vdc/tnlEndPoints/tnlEndPoint?id=%s/tnlConnections/tnlConnection?host=%s&portName=%s/connBindings/connBinding";
    public final static String S_NETWORKID ="networkID";
    public final static String S_VLAN ="vlan";
    ...
    static {
        ...
        criterions.put("networkID", "{'data-type':'STRING','min':'1','max':'36','example':'string','access':'readCreate','key':'true'}");        
        criterions.put("vlan", "{'data-type':'USHORT','min-val':'0','max-val':'4094','example':'5','access':'readCreate'}");        
	}
    ...
    public class OneBody{
        ...
        public void setVlan(String vlan){
           ...
           if(RestUtil.validata(getCriterion(S_VLAN), vlan))
           {
               this.vlan = vlan;
               if(!checkAccessRead(S_VLAN))
               {
                   readAccess = false;
                   System.out.println("set vlan success");
               }
           }else{
               System.out.println( "vlan validation failed, please check again");
           }
        }
		...
		{  
			...
			if (bodyType == BodyType.APPLICATION_JSON)
			{
                ...
                if(!isWrite || checkAccessWrite(S_VLAN))
	            buf.append(",{\"vlan\": \"").append(this.vlan).append("\"}");
			}
		    ...
		    if (bodyType == BodyType.APPLICATION_XML)
		    {
		    	...
		    	if (!isWrite || (RestUtil.isNotEmpty(this.vlan)) && checkAccessWrite("vlan"))
		    	{ buf.append("<vlan>").append(this.vlan).append("</vlan>\n"); }
		    }
		...
		public boolean validate()
		{
		     ...
		     if (checkIsKey(S_VLAN) && RestUtil.isEmpty(this.vlan)) return false;
             return true;
		}
    ...
    public VdcConnBinding(OpsRestCaller rest) {
        this.rest = rest;
    }
    ...
    public String getUrlBody() {
           ...
           lstKeyArr.add(this.host);
           lstKeyArr.add(this.portName);
    ...
    public void parseRestful(RetRpc restfulJson) throws Exception
    {
       ...
       this.setIndexint(0);
        ...
        if(null == restfulJson)
        {
          throw new Exception(restfulJson+" error in VdcConnBinding.java");
        }
        if(200==restfulJson.getStatusCode())
        {
        	if (BodyType.APPLICATION_JSON == bodyType)
        	{
        		...
        		xmlHelper.configure(jsonObject, fullUrlbody, this, VdcConnBinding.OneBody.class);
        		
        	} else {
        		Document doc = RestUtil.getDoc(restfulJson.getContent());
        		OpsXMLHelper xmlHelper = new OpsXMLHelper();
        		xmlHelper.configure(doc.getDocumentElement(), fullUrlbody, this, VdcConnBinding.OneBody.class);
        	}
        }
        else
        {
    ...
    private void setAttributes(JSONArray attributeArray)
    {
        ...
        while(iter.hasNext())
        {
            ...
            if(json.containsKey(S_VLAN))
            {body.setVlan(json.getString(S_VLAN));}
         }
     }
    ...
    public boolean setBody(JSONArray array)
    {
         ...
         for(int i  =0; i <  icount; i ++ )
	     {
            ...
            if (object.get(S_VLAN) != null)
	    	{
	    	    body.setVlan(object.getString(S_VLAN));
	    	}
    ...
    private Boolean getAllAccessWrite()
    {
         ...
         this.readAccess = (this.readAccess && this.checkAccessRead(S_VLAN));
    ...
    */
	...
	{
        VdcConnBinding newond = new VdcConnBinding(OpsServiceManager.getInstance().getDefaultOpsRestCall());
        try {
