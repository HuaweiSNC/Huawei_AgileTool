...
public class VdcTenant {

    public final static String urlBody = "/vdc/tenants/tenant";
    public final static String fullUrlbody = "/vdc/tenants/tenant";
    public final static String S_TENANTID ="tenantID";
    public final static String S_TENANTNAME ="tenantName";
    ...
    static {
        ...
        criterions.put("tenantID", "{'data-type':'STRING','min':'1','max':'36','example':'string','key':'true','access':'readCreate'}");        
        criterions.put("tenantName", "{'data-type':'STRING','min':'1','max':'255','example':'string','key':'true','access':'readCreate'}");        
	}
    ...
    public class OneBody{
        ...
        public void setTenantName(String tenantName){
           ...
           if(RestUtil.validata(getCriterion(S_TENANTNAME), tenantName))
           {
               this.tenantName = tenantName;
               if(!checkAccessRead(S_TENANTNAME))
               {
                   readAccess = false;
                   System.out.println("set tenantName success");
               }
           }else{
               System.out.println( "tenantName validation failed, please check again");
           }
        }
		...
		public OneBody clone()
		{
            ...
            cloneBody.setTenantName(this.tenantName);
            return cloneBody;
		}
		...
		{  
			...
			if (bodyType == BodyType.APPLICATION_JSON)
			{
                ...
                if(!isWrite || checkAccessWrite(S_TENANTNAME))
	            buf.append(",{\"tenantName\": \"").append(this.tenantName).append("\"}");
			}
		    ...
		    if (bodyType == BodyType.APPLICATION_XML)
		    {
		    	...
		    	if (!isWrite || (RestUtil.isNotEmpty(this.tenantName)) && checkAccessWrite("tenantName"))
		    	{ buf.append("<tenantName>").append(this.tenantName).append("</tenantName>\n"); }
		    }
		...
		public boolean validate()
		{
		     ...
		     if (checkIsKey(S_TENANTNAME) && RestUtil.isEmpty(this.tenantName)) return false;
             return true;
		}
    ...
    public VdcTenant(OpsRestCaller rest) {
        this.rest = rest;
    }
    ...
    public void parseRestful(RetRpc restfulJson) throws Exception
    {
       ...
       this.setIndexint(0);
        ...
        if(null == restfulJson)
        {
          throw new Exception(restfulJson+" error in VdcTenant.java");
        }
        if(200==restfulJson.getStatusCode())
        {
        	if (BodyType.APPLICATION_JSON == bodyType)
        	{
        		...
        		xmlHelper.configure(jsonObject, fullUrlbody, this, VdcTenant.OneBody.class);
        		
        	} else {
        		Document doc = RestUtil.getDoc(restfulJson.getContent());
        		OpsXMLHelper xmlHelper = new OpsXMLHelper();
        		xmlHelper.configure(doc.getDocumentElement(), fullUrlbody, this, VdcTenant.OneBody.class);
        	}
        }
        else
        {
    ...
    private void setAttributes(JSONArray attributeArray)
    {
        ...
        while(iter.hasNext())
        {
            ...
            if(json.containsKey(S_TENANTNAME))
            {body.setTenantName(json.getString(S_TENANTNAME));}
         }
     }
    ...
    public boolean setBody(JSONArray array)
    {
         ...
         for(int i  =0; i <  icount; i ++ )
	     {
            ...
            if (object.get(S_TENANTNAME) != null)
	    	{
	    	    body.setTenantName(object.getString(S_TENANTNAME));
	    	}
    ...
    private Boolean getAllAccessWrite()
    {
         ...
         this.readAccess = (this.readAccess && this.checkAccessRead(S_TENANTNAME));
    ...
    */
	...
	{
        VdcTenant newond = new VdcTenant(OpsServiceManager.getInstance().getDefaultOpsRestCall());
        try {
