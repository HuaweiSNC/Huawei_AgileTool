...
public class ServiceflowsRedirect {

    public final static String urlBody = "/serviceflows/serviceflow/flowPolicys/flowPolicy/redirect";
    public final static String fullUrlbody = "/serviceflows/serviceflow?id=%s/flowPolicys/flowPolicy/redirect";
    public final static String S_INGRESSINTERFACE ="ingressInterface";
    public final static String S_EGRESSINTERFACE ="egressInterface";
    ...
    static {
        ...
        criterions.put("ingressInterface", "{'data-type':'STRING','example':'string','access':'readCreate'}");        
        criterions.put("egressInterface", "{'data-type':'STRING','example':'string','access':'readCreate'}");        
	}
    ...
    public class OneBody{
        ...
        public void setIngressInterface(String ingressInterface){
           ...
           if(RestUtil.validata(getCriterion(S_INGRESSINTERFACE), ingressInterface))
           {
               this.ingressInterface = ingressInterface;
               if(!checkAccessRead(S_INGRESSINTERFACE))
               {
                   readAccess = false;
                   System.out.println("set ingressInterface success");
               }
           }else{
               System.out.println( "ingressInterface validation failed, please check again");
           }
        }
    	
    	public String getIngressInterface()
        {
            return this.ingressInterface;
        }
        public void setEgressInterface(String egressInterface){
    	
           if(RestUtil.validata(getCriterion(S_EGRESSINTERFACE), egressInterface))
           {
               this.egressInterface = egressInterface;
               if(!checkAccessRead(S_EGRESSINTERFACE))
               {
                   readAccess = false;
                   System.out.println("set egressInterface success");
               }
           }else{
               System.out.println( "egressInterface validation failed, please check again");
           }
        }
		...
		public OneBody clone()
		{
            ...
            cloneBody.setIngressInterface(this.ingressInterface);
            cloneBody.setEgressInterface(this.egressInterface);
            return cloneBody;
		}
		...
		{  
			...
			{
                if(!isWrite || checkAccessWrite(S_INGRESSINTERFACE))
				buf.append("{\"ingressInterface\": \"").append(this.ingressInterface).append("\"}");
                if(!isWrite || checkAccessWrite(S_EGRESSINTERFACE))
	            buf.append(",{\"egressInterface\": \"").append(this.egressInterface).append("\"}");
			}
			 
		    if (bodyType == BodyType.APPLICATION_XML)
		    {
		    	if (!isWrite || (RestUtil.isNotEmpty(this.ingressInterface)) && checkAccessWrite("ingressInterface"))
		    	{ buf.append("<ingressInterface>").append(this.ingressInterface).append("</ingressInterface>\n"); }
		    	if (!isWrite || (RestUtil.isNotEmpty(this.egressInterface)) && checkAccessWrite("egressInterface"))
		    	{ buf.append("<egressInterface>").append(this.egressInterface).append("</egressInterface>\n"); }
		    }
		...
		public boolean validate()
		{
		     ...
		     if (checkIsKey(S_INGRESSINTERFACE) && RestUtil.isEmpty(this.ingressInterface)) return false;
		     if (checkIsKey(S_EGRESSINTERFACE) && RestUtil.isEmpty(this.egressInterface)) return false;
             return true;
		}
    ...
    public ServiceflowsRedirect(OpsRestCaller rest) {
        this.rest = rest;
    }
    ...
    public void parseRestful(RetRpc restfulJson) throws Exception
    {
       ...
       this.setIndexint(0);
        ...
        if(null == restfulJson)
        {
          throw new Exception(restfulJson+" error in ServiceflowsRedirect.java");
        }
        if(200==restfulJson.getStatusCode())
        {
        	if (BodyType.APPLICATION_JSON == bodyType)
        	{
        		...
        		xmlHelper.configure(jsonObject, fullUrlbody, this, ServiceflowsRedirect.OneBody.class);
        		
        	} else {
        		Document doc = RestUtil.getDoc(restfulJson.getContent());
        		OpsXMLHelper xmlHelper = new OpsXMLHelper();
        		xmlHelper.configure(doc.getDocumentElement(), fullUrlbody, this, ServiceflowsRedirect.OneBody.class);
        	}
        }
        else
        {
    ...
    private void setAttributes(JSONArray attributeArray)
    {
        ...
        while(iter.hasNext())
        {
            ...
            if(json.containsKey(S_INGRESSINTERFACE))
            {body.setIngressInterface(json.getString(S_INGRESSINTERFACE));}
 	         
            if(json.containsKey(S_EGRESSINTERFACE))
            {body.setEgressInterface(json.getString(S_EGRESSINTERFACE));}
         }
     }
    ...
    public boolean setBody(JSONArray array)
    {
         ...
         for(int i  =0; i <  icount; i ++ )
	     {
            ...
            if (object.get(S_INGRESSINTERFACE) != null)
	    	{
	    	    body.setIngressInterface(object.getString(S_INGRESSINTERFACE));
	    	}
            if (object.get(S_EGRESSINTERFACE) != null)
	    	{
	    	    body.setEgressInterface(object.getString(S_EGRESSINTERFACE));
	    	}
    ...
    {
         this.readAccess = (this.readAccess && this.checkAccessRead(S_INGRESSINTERFACE));
         this.readAccess = (this.readAccess && this.checkAccessRead(S_EGRESSINTERFACE));
    ...
    */
	...
	{
        ServiceflowsRedirect newond = new ServiceflowsRedirect(OpsServiceManager.getInstance().getDefaultOpsRestCall());
        try {
