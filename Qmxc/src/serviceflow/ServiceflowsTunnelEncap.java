...
public class ServiceflowsTunnelEncap {

    public final static String urlBody = "/serviceflows/serviceflow/flowPolicys/flowPolicy/redirect/tunnelEncap";
    public final static String fullUrlbody = "/serviceflows/serviceflow?id=%s/flowPolicys/flowPolicy?policyId=%s/redirect/tunnelEncap";
    public final static String S_ENCAPTYPE ="encapType";
    public final static String S_REMARKVALUE ="remarkValue";
    ...
    static {
        ...
        criterions.put("encapType", "{'data-type':'ENUM','expected-values':'vlan','example':'vlan','key':'true','access':'readCreate'}");        
        criterions.put("remarkValue", "{'data-type':'UINT32','min-val':'0','max-val':'4294967295','example':'7','access':'readCreate'}");        
	}
    ...
    public class OneBody{
        ...
        public void setEncapType(String encapType){
           ...
           if(RestUtil.validata(getCriterion(S_ENCAPTYPE), encapType))
           {
               this.encapType = encapType;
               if(!checkAccessRead(S_ENCAPTYPE))
               {
                   readAccess = false;
                   System.out.println("set encapType success");
               }
           }else{
               System.out.println( "encapType validation failed, please check again");
           }
        }
        ...
        public void setRemarkValue(String remarkValue){
           ...
           if(RestUtil.validata(getCriterion(S_REMARKVALUE), remarkValue))
           {
               this.remarkValue = remarkValue;
               if(!checkAccessRead(S_REMARKVALUE))
               {
                   readAccess = false;
                   System.out.println("set remarkValue success");
               }
           }else{
               System.out.println( "remarkValue validation failed, please check again");
           }
        }
		...
		public OneBody clone()
		{
            ...
            cloneBody.setEncapType(this.encapType);
            cloneBody.setRemarkValue(this.remarkValue);
            return cloneBody;
		}
		...
		{  
			...
			{
                if(!isWrite || checkAccessWrite(S_ENCAPTYPE))
				buf.append("{\"encapType\": \"").append(this.encapType).append("\"}");
                if(!isWrite || checkAccessWrite(S_REMARKVALUE))
	            buf.append(",{\"remarkValue\": \"").append(this.remarkValue).append("\"}");
			}
			 
		    if (bodyType == BodyType.APPLICATION_XML)
		    {
		    	if (!isWrite || (RestUtil.isNotEmpty(this.encapType)) && checkAccessWrite("encapType"))
		    	{ buf.append("<encapType>").append(this.encapType).append("</encapType>\n"); }
		    	if (!isWrite || (RestUtil.isNotEmpty(this.remarkValue)) && checkAccessWrite("remarkValue"))
		    	{ buf.append("<remarkValue>").append(this.remarkValue).append("</remarkValue>\n"); }
		    }
		...
		public boolean validate()
		{
		     ...
		     if (checkIsKey(S_ENCAPTYPE) && RestUtil.isEmpty(this.encapType)) return false;
		     if (checkIsKey(S_REMARKVALUE) && RestUtil.isEmpty(this.remarkValue)) return false;
             return true;
		}
    ...
    public ServiceflowsTunnelEncap(OpsRestCaller rest) {
        this.rest = rest;
    }
    ...
    public void parseRestful(RetRpc restfulJson) throws Exception
    {
       ...
       this.setIndexint(0);
        ...
        if(null == restfulJson)
        {
          throw new Exception(restfulJson+" error in ServiceflowsTunnelEncap.java");
        }
        if(200==restfulJson.getStatusCode())
        {
        	if (BodyType.APPLICATION_JSON == bodyType)
        	{
        		...
        		xmlHelper.configure(jsonObject, fullUrlbody, this, ServiceflowsTunnelEncap.OneBody.class);
        		
        	} else {
        		Document doc = RestUtil.getDoc(restfulJson.getContent());
        		OpsXMLHelper xmlHelper = new OpsXMLHelper();
        		xmlHelper.configure(doc.getDocumentElement(), fullUrlbody, this, ServiceflowsTunnelEncap.OneBody.class);
        	}
        }
        else
        {
    ...
    private void setAttributes(JSONArray attributeArray)
    {
        ...
        while(iter.hasNext())
        {
            ...
            if(json.containsKey(S_ENCAPTYPE))
            {body.setEncapType(json.getString(S_ENCAPTYPE));}
 	         
            if(json.containsKey(S_REMARKVALUE))
            {body.setRemarkValue(json.getString(S_REMARKVALUE));}
         }
     }
    ...
    public boolean setBody(JSONArray array)
    {
         ...
         for(int i  =0; i <  icount; i ++ )
	     {
            ...
            if (object.get(S_ENCAPTYPE) != null)
	    	{
	    	    body.setEncapType(object.getString(S_ENCAPTYPE));
	    	}
            if (object.get(S_REMARKVALUE) != null)
	    	{
	    	    body.setRemarkValue(object.getString(S_REMARKVALUE));
	    	}
    ...
    {
         this.readAccess = (this.readAccess && this.checkAccessRead(S_ENCAPTYPE));
         this.readAccess = (this.readAccess && this.checkAccessRead(S_REMARKVALUE));
    ...
    */
	...
	{
        ServiceflowsTunnelEncap newond = new ServiceflowsTunnelEncap(OpsServiceManager.getInstance().getDefaultOpsRestCall());
        try {
